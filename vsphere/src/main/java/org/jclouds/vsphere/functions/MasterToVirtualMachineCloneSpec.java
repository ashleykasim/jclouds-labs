...
public class MasterToVirtualMachineCloneSpec implements Function<VirtualMachine, VirtualMachineCloneSpec> {
   ...
   @Inject
   public MasterToVirtualMachineCloneSpec(
           ResourcePool resourcePool,
           Datastore datastore,
           String cloningStrategy,
           String linuxName,
           boolean postConfiguration,
           String staticIp,
           String subnetMask,
           String dnsDomain,
           String[] gateway,
   ...
   public VirtualMachineCloneSpec apply(VirtualMachine master) {
      return prepareCloneSpec(master, resourcePool, datastore, linuxName, postConfiguration, staticIp, subnetMask, dnsDomain, dnsServerList, gateway);
   }

   private VirtualMachineCloneSpec prepareCloneSpec(VirtualMachine master,
                                                    ResourcePool resourcePool,
                                                    Datastore datastore,
                                                    String linuxName,
                                                    boolean postConfiguration,
      ...
      try {
         cloneSpec = checkNotNull(configureVirtualMachineCloneSpec(relocateSpec, linuxName, postConfiguration, staticIp, subnetMask, dnsDomain, gateway, dnsServerList), "cloneSpec");
      } catch (Exception e) {
   ...
   private VirtualMachineCloneSpec configureVirtualMachineCloneSpec(VirtualMachineRelocateSpec rSpec,
                                                                    ...
                                                                    String dnsDomain,
                                                                    String[] gateway,
                                                                    String[] dnsServerList) throws Exception {
      ...
      if (postConfiguration) {
         ...
         CustomizationIPSettings adptr = new CustomizationIPSettings();
         if (isEmpty(staticIp)) {
            adptr.setIp(new CustomizationDhcpIpGenerator());
         } else {
            CustomizationFixedIp fixedIp = new CustomizationFixedIp();
            fixedIp.setIpAddress(staticIp);
            adptr.setIp(fixedIp);
            adptr.setDnsDomain(dnsDomain);
            adptr.setGateway(gateway);
            adptr.setSubnetMask(subnetMask);
            adptr.setDnsServerList(dnsServerList);
         }
         nicSettingMap[0].adapter = adptr;
