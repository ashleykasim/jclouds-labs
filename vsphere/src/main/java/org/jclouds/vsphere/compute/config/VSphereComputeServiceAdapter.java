      ...
      try (VSphereServiceInstance instance = this.serviceInstance.get();
        ...
        // VSphereTemplateOptions vOptions = VSphereTemplateOptions.class.cast(template.getOptions());
         ...
         VirtualMachineCloneSpec cloneSpec = new MasterToVirtualMachineCloneSpec(resourcePool,
                 sphereHost.getDatastore(),
                 VSphereApiMetadata.defaultProperties().getProperty(CLONING),
                 name,
                 vOptions.postConfiguration(),
                 vOptions.staticIp(),
                 vOptions.subnetMask(),
                 vOptions.dnsDomain(),
                 vOptions.gateway(),
                 vOptions.dnsServerList()).apply(master);
   ...
   private List<VirtualDeviceConfigSpec> configureVmHardware(String name, Template template, VirtualMachine master, VSphereTemplateOptions vOptions, Set<NetworkConfig> networkConfigs) {
      ...
      for (VirtualDevice device : master.getConfig().getHardware().getDevice()) {
         if (device instanceof VirtualDisk) {
            ...
            if (StringUtils.equals(vd.getDeviceInfo().getLabel(), "Hard disk 1")){
               freeDiskCapacity = vd.getCapacityInKB();
            }
            long thisDiskCapacity = vd.getCapacityInKB();
            totalDiskCapacity = totalDiskCapacity + thisDiskCapacity;
            diskNum++;
      ...
      if (vOptions.secondaryDisk() != null) {
         long capacityToAdd =  (vOptions.secondaryDisk() * 1024 * 1024) - (totalDiskCapacity - freeDiskCapacity);
         if (capacityToAdd < 0){
            throw new RuntimeException("Storage cannot be decreased or no additional storage requested");
         } else if (capacityToAdd == 0){
            return null;
         }
//         VirtualController controller = (VirtualController) device;
         ...
         diskfileBacking.setFileName("[" + dsName + "]" + name + "_" + diskNum + "/" + name + "-1.vmdk");
         diskfileBacking.setDiskMode("persistent");
         disk.setBacking(diskfileBacking);
         disk.setControllerKey(1000);
         disk.setUnitNumber(diskNum + 1);
         disk.setKey(diskNum + 1);
         disk.setCapacityInKB(capacityToAdd);
